...
//↓↓↓XML Save Start Arwen ++ 1001006↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
bool CIniFileA::Save_HEAD_XML( const std::string& fileName,int typemode,std::string Section )
{
    #ifdef _CINIFILE_DEBUG
        std::cout <<  "CIniFileA::Save_HEAD_XML() - " << fileName << std::endl;
    #endif
    ...
    if(typemode == 0){
        output.open(fileName.c_str(), std::ios::trunc);//restart
    } else{
	...
	if( !output.is_open() )
        ...
        sSection = Section;
		output << sSection << _CRLFA;
    //Save_HEAD_XML(output);
    //std::cout <<  "Save_HEAD_XML Writing Section " << sSection << std::endl;
...
bool CIniFileA::Save_INT_XML( const std::string& fileName,std::string Section1,std::string Section2,std::string stmp )
{
    #ifdef _CINIFILE_DEBUG
        std::cout <<  "CIniFileA::Save_HEAD_XML() - " << fileName << std::endl;
    #endif
    ...
    //Save_INT_XML(output);
    std::string sSection;
    sSection = Section1 +stmp+Section2;
    output << sSection << _CRLFA;
	output.close();
	return true;
}
//↑↑↑XML Save END Arwen ++ 1001006↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
