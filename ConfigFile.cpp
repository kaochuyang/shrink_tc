	...
	: myDelimiter(delimiter), myComment(comment), mySentry(sentry)
{
	...
	if( !in ) input.open( filename.c_str() , std::ios::binary);//throw file_not_found( filename );
...
bool ConfigFile::writeCf(){
	std::ofstream outcf(myFilename.c_str());
	if( !outcf ) throw file_not_found( myFilename );
	outcf << (*this);
	return true;
}
