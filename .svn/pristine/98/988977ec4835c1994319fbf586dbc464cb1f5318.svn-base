...
#include "SMEM.h"
//---------------------------------------------------------------------------
ScreenActuateArwenManyStatus2 screenActuateArwenManyStatus2;
//---------------------------------------------------------------------------
ScreenActuateArwenManyStatus2::ScreenActuateArwenManyStatus2(void)
{
    loadBitmapFromFile();
    initDispWord();
    cSelect=0;
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::loadBitmapFromFile(void)
{
try {
    FILE *bitmap;
    bitmap=fopen("//cct//bitmap//backGround//actmenu//ActuateArwenManyStatusBitmap.bit","rb");
    if (bitmap) {
        fread(ucActuateArwenBitmap_open,3840,1,bitmap);
        fclose(bitmap);
    }

    bitmap=fopen("//cct//bitmap//backGround//actmenu//ActuateArwenManyStatusBitmap2.bit","rb");
       if (bitmap) {
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::DisplayActuateArwenManyStatus(void)
{
try {
    smem.SetcFace(cACTUATEARWENMANYSTATUS2);
...
//---------------------------------------------------------------------------  //not used
void ScreenActuateArwenManyStatus2::vRefreshEverySec(void)
{
try {
  int iTmpFace = smem.GetcFace();

  if( smem.vGetUCData(TC_CCT_Send_ActuateArwen_Protocal) > 0 || iTmpFace == cACTUATEARWENMANYSTATUS2) {
    vRefreshActuateArwenManyStatusData();
    //vRefreshActSwitch();
  }

  if(iTmpFace == cACTUATEARWENMANYSTATUS2) {
...
//----------------------------------------------------------
void ScreenActuateArwenManyStatus2::vSendCCTProtocol5F81ActArwen(void)
{
try {
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::vShowToPanel(void)
{
try {
...
//    lcd240x128.DISPLAY_GRAPHIC_XY(markStepSecCount[3].X,markStepSecCount[3].Y,word8x16[i4],markStepSecCount[3].height,markStepSecCount[3].width/8);
  ...
  unsigned short int iRunSegActMode = smem.getRunSegmentActMode();
  unsigned short int iSegActType = smem.getSegmentActType(iRunSegActMode);
  ...
  if((iRunSegActMode !=12 && iRunSegActMode<10 )&& (iSegActType==1 || iSegActType==2) ){
    //printf("SegmentActExtendSec:%d iCurrentSubphase:%d SegmentZoneTimes:%d\n",smem.getSegmentActExtendSec(iRunSegActMode,iCurrentSubphase),iCurrentSubphase,smem.getSegmentZoneTimes(iRunSegActMode));
    if(smem.getSegmentActVDPhaseTriggerSwitch(iRunSegActMode,iCurrentSubphase)>=1){
        iHiTMP = smem.getSegmentActExtendSec(iRunSegActMode,iCurrentSubphase);
        iLowTMP = smem.getSegmentZoneTimes(iRunSegActMode);
        lcd240x128.DISPLAY_GRAPHIC_XY(markExtend.X,markExtend.Y,word8x16[iHiTMP],markExtend.height,markExtend.width/8);
        lcd240x128.DISPLAY_GRAPHIC_XY(markGapOut.X,markGapOut.Y,word8x16[iLowTMP],markGapOut.height,markGapOut.width/8);
    }else{
        lcd240x128.DISPLAY_GRAPHIC_XY(markExtend.X,markExtend.Y,word8x16[0],markExtend.height,markExtend.width/8);
        lcd240x128.DISPLAY_GRAPHIC_XY(markGapOut.X,markGapOut.Y,word8x16[0],markGapOut.height,markGapOut.width/8);
    }
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::vRefreshActuateArwenManyStatusData(void)
{
try {
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::vRefreshActSwitch(void)
{
try {
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::vCalActTuningStatus(void)
{
try {
...
//testMark  if(bActTuningStatusNow == true) {
    ...
    iDnyActCount = smem.vGetCurrentActCountAddVal();
    printf("iDnyActCount:%d\n", iDnyActCount);
    iDnyActEnd = smem.vGetCurrentActEndVal();
    printf("iDnyActEnd:%d\n", iDnyActEnd);
//  }
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::vShowActTuningStatus(void)
{
try {
  ...
  */
  iHiTMP = iDnyActCount/100;//觸動次數
  iMedianTMP =iDnyActCount / 10;
  iLowTMP = iDnyActCount % 10;
  lcd240x128.DISPLAY_GRAPHIC_XY(markActCount[0][0].X,markActCount[0][0].Y,word8x16[iHiTMP],markActCount[0][0].height,markActCount[0][0].width/8);
  lcd240x128.DISPLAY_GRAPHIC_XY(markActCount[0][1].X,markActCount[0][1].Y,word8x16[iMedianTMP],markActCount[0][1].height,markActCount[0][1].width/8);
  lcd240x128.DISPLAY_GRAPHIC_XY(markActCount[0][2].X,markActCount[0][2].Y,word8x16[iLowTMP],markActCount[0][2].height,markActCount[0][2].width/8);

  iHiTMP = iDnyActEnd % 10;//結束模式
  lcd240x128.DISPLAY_GRAPHIC_XY(markActEndMode[0].X,markActEndMode[0].Y,word8x16[iHiTMP],markActEndMode[0].height,markActEndMode[0].width/8);
  ...
  for(int i=1;i<4;i++){
	  ...
	  */
	  if((i-1) <=switch_times){
	  iHiTMP = ilastActCount[i-1] / 100; //觸動次數
      iMedianTMP =ilastActCount[i-1] / 10;
      iLowTMP = ilastActCount[i-1] % 10;
      if(ilastActCount[i-1]>=0){
		  lcd240x128.DISPLAY_GRAPHIC_XY(markActCount[i][0].X,markActCount[i][0].Y,word8x16[iHiTMP],markActCount[i][0].height,markActCount[i][0].width/8);
		  lcd240x128.DISPLAY_GRAPHIC_XY(markActCount[i][1].X,markActCount[i][1].Y,word8x16[iMedianTMP],markActCount[i][1].height,markActCount[i][1].width/8);
		  lcd240x128.DISPLAY_GRAPHIC_XY(markActCount[i][2].X,markActCount[i][2].Y,word8x16[iLowTMP],markActCount[i][2].height,markActCount[i][2].width/8);
	  }
	  iHiTMP = ilastActEndMode[i-1] % 10; //結束模式 Gap_out or Max_out
      if(ilastActEndMode[i-1]>=0){
		  lcd240x128.DISPLAY_GRAPHIC_XY(markActEndMode[i].X,markActEndMode[i].Y,word8x16[iHiTMP],markActEndMode[i].height,markActEndMode[i].width/8);
	  }
  ...
  */
  ...
  lcd240x128.DISPLAY_GRAPHIC_XY(markMinGreenTime[1].X,markMinGreenTime[1].Y,word8x16[iMedianTMP],markMinGreenTime[1].height,markMinGreenTime[1].width/8);
  lcd240x128.DISPLAY_GRAPHIC_XY(markMinGreenTime[2].X,markMinGreenTime[2].Y,word8x16[iLowTMP],markMinGreenTime[2].height,markMinGreenTime[2].width/8);
   ...
   */
   ...
   lcd240x128.DISPLAY_GRAPHIC_XY(markMaxGreenTime[1].X,markMaxGreenTime[1].Y,word8x16[iMedianTMP],markMaxGreenTime[1].height,markMaxGreenTime[1].width/8);
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::initDispWord(void)
{
try {
...
//---------------
    ...
    for(int i = 0; i < 4; i++) {
      ...
      markActCount[i][j].width = 8; markActCount[i][j].height = 16;
...
//---------------
    ...
    for(int i=0;i<4;i++){
    	markActTuning[i].X = 96+(i*40); markActTuning[i].Y = 62;//動態調整
    	markActTuning[i].width = 8;    markActTuning[i].height = 16;
    	markActEndMode[i].width = 8; markActEndMode[i].height =16;//結束模式
    	markActEndMode[i].X = 96+(i*40); markActEndMode[i].Y = 48;
    }
    ...
    markActCount[i][0].X = 90+(i*40); markActCount[i][0].Y = 32;//觸動次數
    markActCount[i][1].X = 98+(i*40); markActCount[i][1].Y = 32;
    markActCount[i][2].X = 106+(i*40); markActCount[i][2].Y = 32;
    ...
    //------------------------------
    markMinGreenTime[0].width = 8; markMinGreenTime[0].height = 16;
    markMinGreenTime[1].width = 8; markMinGreenTime[1].height = 16;
    markMinGreenTime[2].width = 8; markMinGreenTime[2].height = 16;

    markMaxGreenTime[0].width = 8; markMaxGreenTime[0].height = 16;
    markMaxGreenTime[1].width = 8; markMaxGreenTime[1].height = 16;
    markMaxGreenTime[2].width = 8; markMaxGreenTime[2].height = 16;

    markMinGreenTime[0].X = 48; markMinGreenTime[0].Y = 80;
    markMinGreenTime[1].X = 56; markMinGreenTime[1].Y = 80;
    markMinGreenTime[2].X = 64; markMinGreenTime[2].Y = 80;

    markMaxGreenTime[0].X = 112; markMaxGreenTime[0].Y = 80;
    markMaxGreenTime[1].X = 120; markMaxGreenTime[1].Y = 80;
    markMaxGreenTime[2].X = 128; markMaxGreenTime[2].Y = 80;

    markExtend.width = 8;markExtend.height = 16;
    markExtend.X = 188; markExtend.Y = 80;
    markGapOut.width = 8;markGapOut.height = 16;
    markGapOut.X = 232; markGapOut.Y = 80;

  } catch (...) {}
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyWork(BYTE key)
{
try {
    switch (key) {
        case 0x80:
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey0Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey1Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey2Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey3Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey4Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey5Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey6Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey7Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey8Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKey9Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyAWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyBWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyCWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyDWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyEWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyFWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyF1Work(void)
{
//    screenLogo.DisplayLogo();
    screenMain.DisplayMain();
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyF2Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyF3Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyF4Work(void)
{
//    screenMain.DisplayMain();
	screenActuateArwenMenu.DisplayActuateArwenMenu();
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyUPWork(void)
{
try {
  } catch (...) {}
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyDOWNWork(void)
{
try {
  } catch (...) {}
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyLEFTWork(void)
{
try {
  } catch (...) {}
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyRIGHTWork(void)
{
try {
  } catch (...) {}
}
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::doKeyEnterWork(void)
{
    switch (cSelect+1) {
        case 1:
          doKey1Work();
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::vRefreshActuateTimes()
{
try {

	if(smem.GetcFace()==cACTUATEARWENMANYSTATUS2){
		...
		//if((RecordPhaseGreen !=siCurrentPhaseGreen) && (siCurrentPhaseGreen!=9999) && (RecordPhaseGreen!=9999)/*(iAdjGreenSec >=siCurrentPhaseGreen)*/ ) {
			...
			if(switch_times==0){
				...
				ilastActCount[i]=RecordDnyActCount;//觸動次數
				ilastActEndMode[i]=RecordDnyActEnd;//結束模式
				...
				ilastActCount[i]=ilastActCount[i-1];//觸動次數
				ilastActEndMode[i]=ilastActEndMode[i-1];//結束模式
				...
				ilastActCount[i-1]=RecordDnyActCount;//觸動次數
				ilastActEndMode[i-1]=RecordDnyActEnd;//結束模式
				...
				ilastActCount[i-1]=ilastActCount[i-2];//觸動次數
				ilastActEndMode[i-1]=ilastActEndMode[i-2];//結束模式
			    ...
			    ilastActCount[i-2]=RecordDnyActCount;//觸動次數
				ilastActEndMode[i-2]=RecordDnyActEnd;//結束模式
			    ...
			    ilastActCount[i-2]=ilastActCount[i-3];//觸動次數
				ilastActEndMode[i-2]=ilastActEndMode[i-3];//結束模式
			    ...
			    ilastActCount[i-3]=RecordDnyActCount;//觸動次數
				ilastActEndMode[i-3]=RecordDnyActEnd;//結束模式
			...
			for(int j=0;j<4;j++){
                ...
                printf("ilastActCount[%d]:%d ilastActEndMode[%d]:%d\n",j,ilastActCount[j],j,ilastActEndMode[j]);
			}
		}
	...
	//arwen ++
	RecordDnyActCount=iDnyActCount;
	RecordDnyActEnd=iDnyActEnd;
...
//---------------------------------------------------------------------------
void ScreenActuateArwenManyStatus2::vDisplayActionChange(void)
{
