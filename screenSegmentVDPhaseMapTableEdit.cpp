...
ScreenSegmentVDPhaseMapTableEdit screenSegmentVDPhaseMapTableEdit;
//---------------------------------------------------------------------------
ScreenSegmentVDPhaseMapTableEdit::ScreenSegmentVDPhaseMapTableEdit(void)
{
    LoadBitmapFromFile();
    InitDispWord();
    phase_id=0;
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyWork(BYTE key)                                           //已�\uFFFD�\uFFFD\uFFFD��\uFFFD\uFFFD\uFFFD\uFFFD0x80~0x98�\uFFFD\uFFFD
{
try {
    switch (key) {
        case 0x80:
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::LoadBitmapFromFile(void)
{
try {
    FILE *bitmap;
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DisplayVDPhaseMapTableEdit(int _tmpid)
{
try {
    ...
    for(int i=0;i<8;i++){
        ...
        for(int j=0;j<8;j++){
        	...
        	//usiLCN[i][j]=smem.getSegmentActVDPhaseMap(_tmpid,i,j);
            //printf("ScreenSegmentVDPhaseMapTableEdit::DisplayVDPhaseMapTableEdit phase%d_%d=%d\n",i,j,usiLCN[i][j]);
            for(k=_tmpcount;k<_tmpcount+5;k++){
			if(k%5==0){
				value[i][k]=(usiLCN[i][j]/10000);
				printf("--value[%d][%d]=%d ",i,k,value[i][k]);
			}else if(k%5 ==1){
				value[i][k]=(usiLCN[i][j]/1000)%10;
				printf("--value[%d][%d]=%d ",i,k,value[i][k]);
			}else if(k%5==2){
				value[i][k]=(usiLCN[i][j]%1000)/100;
				printf("--value[%d][%d]=%d ",i,k,value[i][k]);
			}else if(k%5==3){
				value[i][k]=(usiLCN[i][j]%100)/10;
				printf("--value[%d][%d]=%d ",i,k,value[i][k]);
			}else if(k%5==4){
				value[i][k]=usiLCN[i][j]%10;
				printf("--value[%d][%d]=%d ",i,k,value[i][k]);
			}
            }
            _tmpcount=k;
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::InitDispWord(void)
{
try {
	for(int i=0;i<2;i++){
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DisplahPhaseId()
{
try{
	int tmp_ip[2];
...
void ScreenSegmentVDPhaseMapTableEdit::DisplayVDTableEdit(void)
{
try {
	/*
	int _tmpcount=0,j=0;
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DisplaySaveValue()
{
try{
	printf("ScreenSegmentVDPhaseMapTableEdit::DisplaySaveValue phase_id:%d\n",phase_id);
	for(int i=0;i<40;i++ ){
	    //if(phase_id-1<0){usiLCN[0][i] = value[0][i];}else{usiLCN[phase_id-1][i] = value[phase_id-1][i];}
...
void ScreenSegmentVDPhaseMapTableEdit::DoKey0Work(void)
{
try {
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey1Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey2Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey3Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey4Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey5Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey6Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey7Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey8Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKey9Work(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyAWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyBWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyCWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyDWork(void)
{
}
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyEWork(void)
{
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyFWork(void)
{
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyF1Work(void)
{
try {
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyF2Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyF3Work(void)
{
}
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyF4Work(void)
{
try {
	//screenVDPhaseMapTable.DisplayVDPhaseMapTable();
	//screenActuateStrategyMenu.DisplayActuateStrategyMenu();
	screenActuatePlaySwitch.DisplayActType(lastActPlaySwitchMode);
  } catch (...) {}
}
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyEnterWork(void)
{
try {
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyUPWork(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyDOWNWork(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyLEFTWork(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
...
//---------------------------------------------------------------------------
void ScreenSegmentVDPhaseMapTableEdit::DoKeyRIGHTWork(void)
{
try {
    clearCursor8x16(position[cPosition].X,position[cPosition].Y+16);
    cPosition++;
